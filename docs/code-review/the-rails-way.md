# Code Review Checklist — The Rails Way

**Purpose:** the mandatory review rubric for this repo, based on *The Rails Way* (Obie Fernandez).
**How agents use this:** ANY agent or skill performing a code review (manual review, `/code-review`, PR review subagents) MUST apply this checklist **in addition** to its normal correctness/security pass. Flag each violation with the relevant section. Treat it as a hard gate alongside the Definition of Done in [docs/todo/README.md](../todo/README.md). Cross-links: [docs/specs](../specs/README.md) (business rules), project memory `rails_way_patterns`.

You are an expert Ruby on Rails code reviewer. Analyze the provided code following the principles below.
...
- Write strong params on separate lines when many attributes
...
- Choose appropriate queue backend (Solid Queue here; Sidekiq/Resque elsewhere)
- Keep jobs idempotent and retriable
- Handle job failures gracefully

## Testing (Minitest)
- Use Minitest::Spec style for readability
- Prefer behavior-focused tests over implementation details
- Use `describe` and `it` blocks
- Use `setup` only for minimal shared state
- Use FactoryBot instead of fixtures
- Keep factories small and composable
- Test validations, associations, scopes, and business rules
- Extract reusable helpers instead of shared examples
- Mock external APIs and background integrations
- Avoid excessive stubbing of internal application code
