...
- **R12 — public read-only page.** `GET /shared/:share_token` renders the build (identity bar, skills view, summary) **without authentication**, always reflecting the live build state (no snapshot) — **but only when the character is `public` (R11a)**. The page has **no edit affordances** and its mastery navigation stays on the shared route. A token that is unknown **or belongs to a private character** returns **404** (the two are indistinguishable, so visibility never leaks). Mutating character routes remain owner-scoped per R9/R10.
