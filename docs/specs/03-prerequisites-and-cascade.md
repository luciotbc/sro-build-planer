...
## Current code gap (informs backlog)
- Today `PrerequisiteResolver`, `update_mastery`, `sync_character_level`, and `find_blocking_dependents` operate **only on the current side** (`current_skill_level` / `current_mastery_level` / `current_level`). The planned side is stored without cascade.
- **Work item:** generalize the resolver + services to take a `side` parameter and apply R3–R6 to the matching fields. Tracked in backlog (resolver refactor under 001; planned-side wiring under the editor/mastery tasks 009–011).
