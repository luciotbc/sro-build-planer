...
races = YAML.safe_load(ERB.new(File.read(fixture_path.join("races.yml")).to_s).result, aliases: true)
          .transform_values do |attrs|
  Race.find_or_create_by!(external_id: attrs["external_id"], name: attrs["name"])
end

masteries = YAML.safe_load(ERB.new(File.read(fixture_path.join("masteries.yml")).to_s).result, aliases: true)

masteries.each do |_fixture_name, attrs|
  Mastery.find_or_initialize_by(external_id: attrs["external_id"]).tap do |mastery|
